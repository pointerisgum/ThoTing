...
#define FTDefaultTintColor                  [UIColor colorWithRed:255.f/255.f green:255.f/255.f blue:255.f/255.f alpha:1.f]
#define FTDefaultTextColor                  [UIColor blackColor]
#define FTDefaultMenuFont                   [UIFont systemFontOfSize:14.f]
#define FTDefaultMenuWidth                  [[UIScreen mainScreen] bounds].size.width - 10
#define FTDefaultMenuIconSize               24.f
#define FTDefaultMenuRowHeight              50.f
...
@property (nonatomic, strong) UIButton *btn_Title;
@end
...
{
    if (!_btn_Title) {
        _btn_Title = [UIButton buttonWithType:UIButtonTypeCustom];
        _btn_Title.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
        _btn_Title.userInteractionEnabled = NO;
        [_btn_Title.titleLabel setFont:[UIFont fontWithName:@"Helvetica" size:14.f]];
        [_btn_Title setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [_btn_Title setImage:BundleImage(@"check_black_15.png") forState:UIControlStateSelected];
        _btn_Title.semanticContentAttribute = UISemanticContentAttributeForceRightToLeft;
        _btn_Title.titleEdgeInsets = UIEdgeInsetsMake(0, 20, 0, 0);
        _btn_Title.imageEdgeInsets = UIEdgeInsetsMake(0, 30, 0, 0);
    }
    return _btn_Title;
}
...
//    [self.contentView addSubview:self.menuNameLabel];
    ...
    [self.btn_Title setTitle:menuName forState:UIControlStateNormal];
    [self.contentView addSubview:self.btn_Title];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *str_QDW = [[NSUserDefaults standardUserDefaults] objectForKey:@"QDW"];
    if( [menuCell.btn_Title.titleLabel.text isEqualToString:str_QDW] )
    {
        menuCell.btn_Title.selected = YES;
    }
    else
    {
        menuCell.btn_Title.selected = NO;
    }
          ...
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.iv_Bg = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, KSCREEN_WIDTH, KSCREEN_HEIGHT)];
        self.iv_Bg.backgroundColor = [UIColor blackColor];
        self.iv_Bg.alpha = 0.6f;

//        self.backgroundView.backgroundColor = [UIColor blackColor];
//        self.backgroundView.alpha = 0.6f;
        
        [[self backgroundWindow] addSubview:self.iv_Bg];
